...
- 模型呼叫失敗與缺少 API Key 時，該次使用者輸入及系統通知會在網頁標示為「無效對話」，不成為正式回合；時間、壓縮及模型狀態回復到輸入前，兩則訊息也不進入正文、壓縮或大模型上下文。下一次對話成功後會自動移除先前所有無效對話；啟動時會把舊版本留在故事尾端的失敗回合補上無效標記。Discord Slash 指令會以 ephemeral 私密訊息顯示生成錯誤，伺服器中的普通訊息則改以私訊通知，避免錯誤文字進入公開故事頻道。
