...
環境設定的對話 API Key 可用 `Key 1`、`Key 2...` 分頁管理，每組同時包含主對話 Key 與大模型處理 Key。新故事自動租用最前面的閒置組，任何 AI 呼叫都會續期；24 小時沒有 AI 活動後才供其他故事使用，刪除故事或 Discord 頻道則立即釋放。切換供應商時會保留各供應商自己的 Key 組、輸出模型與 Base URL，並讓現有故事改用新供應商重新分配；目前供應商的專屬 Key 優先，通用 Key 只作舊設定後備，若兩者都沒有則要求先設定。Base URL 可直接填入包含 `/chat/completions` 與查詢參數的完整 deployment URL；此時 API 輸出模型可留空，請求會使用 `api-key` 驗證。環境設定的測試連線使用模型預設溫度，不會送出額外的 `temperature`。

主要模型不能讀圖時，可在環境設定的「圖片處理 API」改用專用圖片模型；圖片會先轉成隱藏文字描述，再交給主要模型。圖片模型有獨立的思考強度與溫度，思考開啟時不送溫度。使用主要模型直接處理時，專用 API 欄位會停用但保留內容；測試連接會實際傳送標準 PNG。專用 Key 只保存在本機，完整設定見 [細節文件](docs/DETAILS.md)。

正文 API 成功但回傳空白內容時，只會重試該次正文請求一次。Discord 頻道中的模型失敗會顯示在原訊息旁，預設 15 秒後自動刪除。
...
啟動時會以狀態檔自身的語言設定載入既有本地及頻道對話，不會在主狀態尚未初始化時讀取全域狀態；已標記的失敗回合也不會在每次啟動重寫。讀取失敗時仍會停止啟動，避免用空白資料覆蓋原檔。
...
聊天長文字會自動換行，寬表格與程式碼可在區塊內捲動，避免撐開聊天區與輸入列。

主頁：

- 角色卡與助手：由彩色流動、霓虹邊框的「選擇」入口瀏覽卡片，或從「建立」選單建立及匯入角色卡；可直接開啟完整表單建立助手，新助手的 Prompt 預設留空，另支援編輯、裁切封面、單卡／全局世界書、可自訂名稱的多分頁開場對話與角色卡匯出。
